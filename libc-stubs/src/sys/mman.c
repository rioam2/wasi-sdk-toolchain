...
/**
 * This file contains an implementation of memory-mapped files (mmap) for WebAssembly/WASI targets
 * The implementation uses malloc and raw pointers to store memory mapped sections of files.
 * Native mmap support should be used when implemented upstream by wasi-libc
 */
...
void *mmap(void *addr, size_t len, int prot, int flags, int fildes, off_t off) {
    void* ptr = malloc(len);
    memset(ptr, 0x00, len);
    std::unordered_map<int, std::pair<void *, size_t> >::iterator itr = mappedFiles.find(fildes);
...
int munmap(void *addr, size_t len) {
    ...
    std::unordered_map<int, std::pair<void*, size_t> >::iterator it = mappedFiles.begin();
